...
bool InsecureSignature::VerifyNative(g1_t *pubKeys, g2_t *mappedHashes, size_t len) {
  ...
  if (gt_cmp(target, candidate) != CMP_EQ || core_get()->code != STS_OK) {
...
InsecureSignature InsecureSignature::Aggregate(const std::vector<InsecureSignature> &sigs) {
  if (sigs.empty()) { throw std::string("sigs must not be empty"); }
...
bool operator!=(InsecureSignature const &a, InsecureSignature const &b) { return !(a == b); }
...
Signature Signature::FromInsecureSig(const InsecureSignature &sig) { return FromG2(&sig.sig); }

Signature Signature::FromInsecureSig(const InsecureSignature &sig, const AggregationInfo &info) {
  return FromG2(&sig.sig, info);
}

Signature::Signature(const Signature &_signature) : sig(_signature.sig), aggregationInfo(_signature.aggregationInfo) {}
...
void Signature::Serialize(uint8_t *buffer) const { sig.Serialize(buffer); }

std::vector<uint8_t> Signature::Serialize() const { return sig.Serialize(); }
...
bool Signature::Verify() const {
  if (GetAggregationInfo()->Empty()) { return false; }
  std::vector<PublicKey> pubKeys = GetAggregationInfo()->GetPubKeys();
  std::vector<uint8_t *> messageHashes = GetAggregationInfo()->GetMessageHashes();
  ...
  std::map<uint8_t *, std::vector<PublicKey>, Util::BytesCompare32> hashToPubKeys;
...
Signature Signature::AggregateSigs(std::vector<Signature> const &sigs) {
  BLS::AssertInitialized();
  ...
  for (size_t i = 0; i < messageHashes.size(); i++) {
    if (pubKeys[i].size() != messageHashes[i].size()) { throw std::string("Lengths of vectors must match."); }
  }
...
Signature Signature::AggregateSigsSecure(std::vector<Signature> const &sigs, std::vector<PublicKey> const &pubKeys,
  ...
  // Raise all signatures to power of the corresponding t's and aggregate the
  // results into aggSig
...
Signature Signature::AggregateSigsInternal(std::vector<Signature> const &sigs,
  ...
  for (size_t i = 0; i < sortKeysSorted.size(); i++) {
    ...
    pubKeysSorted.push_back(PublicKey::FromBytes(sortKey + BLS::MESSAGE_HASH_LEN));
  }
  ...
  // Raise all signatures to power of the corresponding t's and aggregate the
  // results into aggSig Also accumulates aggregation info for each signature
  std::vector<AggregationInfo> infos;
  std::vector<InsecureSignature> expSigs;
...
Signature Signature::DivideBy(std::vector<Signature> const &divisorSigs) const {
  ...
  for (const Signature &divisorSig : divisorSigs) {
    std::vector<PublicKey> pks = divisorSig.GetAggregationInfo()->GetPubKeys();
