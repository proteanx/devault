...
AggregationInfo AggregationInfo::FromMsgHash(const PublicKey &pk, const uint8_t *messageHash) {
  uint8_t *mapKey = new uint8_t[BLS::MESSAGE_HASH_LEN + PublicKey::PUBLIC_KEY_SIZE];
...
AggregationInfo AggregationInfo::FromMsg(const PublicKey &pk, const uint8_t *message, size_t len) {
  uint8_t hash[BLS::MESSAGE_HASH_LEN];
  Util::Hash256(hash, message, len);
  return FromMsgHash(pk, hash);
}

AggregationInfo AggregationInfo::FromVectors(std::vector<PublicKey> const &pubKeys,
...
void AggregationInfo::RemoveEntries(std::vector<uint8_t *> const &messages, std::vector<PublicKey> const &pubKeys) {
  if (messages.size() != pubKeys.size()) { throw std::string("Invalid entries"); }
...
void AggregationInfo::GetExponent(bn_t *result, const uint8_t *messageHash, const PublicKey &pubKey) const {
  uint8_t mapKey[BLS::MESSAGE_HASH_LEN + PublicKey::PUBLIC_KEY_SIZE];
  std::memcpy(mapKey, messageHash, BLS::MESSAGE_HASH_LEN);
  pubKey.Serialize(mapKey + BLS::MESSAGE_HASH_LEN);
  bn_copy(*result, *tree.at(mapKey));
}

std::vector<PublicKey> AggregationInfo::GetPubKeys() const { return sortedPubKeys; }
...
bool operator<(AggregationInfo const &a, AggregationInfo const &b) {
  ...
  for (size_t i = 0; i < a.sortedMessageHashes.size() || i < b.sortedMessageHashes.size(); i++) {
    ...
    if (std::memcmp(bufferA, bufferB, BLS::MESSAGE_HASH_LEN + PublicKey::PUBLIC_KEY_SIZE) < 0) {
      lessThan = true;
      break;
    } else if (std::memcmp(bufferA, bufferB, BLS::MESSAGE_HASH_LEN + PublicKey::PUBLIC_KEY_SIZE) > 0) {
      lessThan = false;
      break;
    }
...
std::ostream &operator<<(std::ostream &os, AggregationInfo const &a) {
  for (auto &kv : a.tree) {
    os << Util::HexStr(kv.first, 80) << ".."
       << ":" << std::endl;
    uint8_t str[RELIC_BN_BYTES * 3 + 1];
    bn_write_bin(str, sizeof(str), *kv.second);
    os << Util::HexStr(str + RELIC_BN_BYTES * 3 + 1 - 5, 5) << std::endl;
  }
  return os;
}
...
void AggregationInfo::InsertIntoTree(AggregationInfo::AggregationTree &tree, const AggregationInfo &info) {
  for (auto &mapEntry : info.tree) {
    uint8_t *messageCopy = new uint8_t[BLS::MESSAGE_HASH_LEN + PublicKey::PUBLIC_KEY_SIZE];
    std::memcpy(messageCopy, mapEntry.first, BLS::MESSAGE_HASH_LEN + PublicKey::PUBLIC_KEY_SIZE);
...
void AggregationInfo::SortIntoVectors(std::vector<uint8_t *> &ms, std::vector<PublicKey> &pks,
                                      const AggregationInfo::AggregationTree &tree) {
  for (auto &kv : tree) {
    ms.push_back(kv.first);
  }
  sort(begin(ms), end(ms), Util::BytesCompare80());
  for (auto &m : ms) {
...
AggregationInfo AggregationInfo::SecureMergeInfos(std::vector<AggregationInfo> const &collidingInfosArg) {
  ...
  std::sort(sortedCollidingInfos.begin(), sortedCollidingInfos.end(),
            [&collidingInfosArg](size_t a, size_t b) { return collidingInfosArg[a] < collidingInfosArg[b]; });
  ...
  for (size_t i = 0; i < sortedCollidingInfos.size(); i++) {
    ...
    for (auto &mapEntry : info.tree) {
      ...
      if (newMapEntry == newTree.end()) {
        ...
        uint8_t *mapKeyCopy = new uint8_t[BLS::MESSAGE_HASH_LEN + PublicKey::PUBLIC_KEY_SIZE];
        std::memcpy(mapKeyCopy, mapEntry.first, BLS::MESSAGE_HASH_LEN + PublicKey::PUBLIC_KEY_SIZE);
