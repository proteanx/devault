 ...
 * Aggregation of these signatures is not secure on it's own, use Signature
 * instead
 */
class InsecureSignature {
  ...
  // anywhere else.
  static bool VerifyNative(g1_t *pubKeys, g2_t *mappedHashes, size_t len);
...
class Signature {
  ...
  static Signature AggregateSigsSecure(std::vector<Signature> const &sigs, std::vector<PublicKey> const &pubKeys,
