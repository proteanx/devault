...
PrivateKey PrivateKey::AggregateInsecure(std::vector<PrivateKey> const &privateKeys) {
  if (privateKeys.empty()) { throw std::string("Number of private keys must be at least 1"); }
...
PrivateKey PrivateKey::Aggregate(std::vector<PrivateKey> const &privateKeys, std::vector<PublicKey> const &pubKeys) {
  if (pubKeys.size() != privateKeys.size()) {
    throw std::string("Number of public keys must equal number of private keys");
  }
  if (privateKeys.empty()) { throw std::string("Number of keys must be at least 1"); }
