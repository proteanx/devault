...
bool operator==(const Coin &a, const Coin &b) {
  ...
  return a.IsCoinBase() == b.IsCoinBase() && a.GetHeight() == b.GetHeight() && a.GetTxOut() == b.GetTxOut();
}
...
TEST_CASE("coins_cache_simulation_test") {
  ...
  for (unsigned int i = 0; i < NUM_SIMULATION_ITERATIONS; i++) {
    // Do a random modification.
    {
      ...
      bool result_havecoin = test_havecoin_before
          ? stack.back()->HaveCoin(COutPoint(txid, 0)) : false;
    ...
    if (InsecureRandRange(100) == 0) {
      ...
      if (stack.size() == 0 || (stack.size() < 4 && InsecureRandBool())) {
        ...
        if (stack.size() == 4) { reached_4_caches = true; }
      }
    }
  }
...
TEST_CASE("updatecoins_simulation_test") {
  ...
  for (int64_t i = 0; i < NUM_SIMULATION_ITERATIONS; i++) {
    ...
    if (randiter % 20 < 19) {
      ...
      // 17/20 times reconnect previous or add a regular tx
        ...
        if (duplicate_coins.count(prevout)) { spent_a_duplicate_coinbase = true; }
      }
      ...
      utxoData.emplace(outpoint, std::make_tuple(CTransaction(tx), undo, old_coin));
    }
    ...
    // 1/20 times undo a previous transaction
      ...
      if (!tx.IsCoinBase()) { result[tx.vin[0].prevout] = orig_coin; }
      ...
      if (!tx.IsCoinBase()) { utxoset.insert(tx.vin[0].prevout); }
    }
    ...
    if (InsecureRandRange(100) == 0) {
      ...
      if (stack.size() == 0 || (stack.size() < 4 && InsecureRandBool())) {
        CCoinsView *tip = &base;
        if (stack.size() > 0) { tip = stack.back(); }
        stack.push_back(new CCoinsViewCacheTest(tip));
      }
    }
  }
...
static const Amount Fail(-3 * Amount::min_amount()); // Capitalize due to Catch
...
class SingleEntryCacheTest {
  public:
  SingleEntryCacheTest(const Amount base_value, const Amount cache_value, char cache_flags) {
...
void CheckAccessCoin(const Amount base_value, const Amount cache_value, const Amount expected_value, char cache_flags,
                     char expected_flags) {
...
void CheckAddCoinBase(Amount base_value, Amount cache_value, Amount modify_value, Amount expected_value,
...
void CheckWriteCoin(Amount parent_value, Amount child_value, Amount expected_value, char parent_flags, char child_flags,
                    char expected_flags) {
