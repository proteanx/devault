...
static UniValue getnewaddress(const Config &config,
    ...
    if (pwallet->UseBLSKeys()) {
        CKeyID<0> keyID = newKey.GetKeyID();
        pwallet->SetAddressBook(keyID, label, "receive");
        return EncodeDestination(keyID);
    } else {
        CKeyID<1> keyID = newKey.GetKeyID1();
        pwallet->SetAddressBook(keyID, label, "receive");
        return EncodeDestination(keyID);
    }
}
...
static UniValue getrawchangeaddress(const Config &config,
    ...
    CKeyID<0> keyID = vchPubKey.GetKeyID();
...
UniValue sweepprivkey(const Config &config, const JSONRPCRequest &request) {
...

    {
        ...
        //std::string p = EncodeCashAddr(key.GetPubKey().GetKeyID(), Params());
...
static UniValue signmessage(const Config &config,
    ...
    const CKeyID<0> *keyID = &std::get<CKeyID<0>>(dest);
    if (!keyID) {
