...
bool CCryptoKeyStore::GetPubKey(const CKeyID<0> &address, CPubKey &vchPubKeyOut) const {
    {
        LOCK(cs_KeyStore);
...
bool CCryptoKeyStore::GetPubKey(const CKeyID<1> &address, CPubKey &vchPubKeyOut) const {
    {
        LOCK(cs_KeyStore);
        // Check for watch-only pubkeys
        return CBasicKeyStore::GetPubKey(address, vchPubKeyOut);
